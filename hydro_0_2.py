...
#common conv nets: no local-connected layers
#samples are cross with centers distributed on a circle
#input are 20*20 images
...
X_train = np.zeros((100,20,20,1))
for k in range(0,100):
    phi = np.random.uniform(0,2*np.pi,1)
    x = int(10 + 6*np.cos(phi))
    y = int(10 + 6*np.sin(phi))
    for i in range(0,20):
        for j in range(0,20):
            if i==x or i==x+1:
                if j>y-3 and j<y+4:
...
def generator(latent_size):
    ...
    x = Dense(5*5*16)(latent)
    x = Reshape((5,5,16))(x)
    x = UpSampling2D(size=(2,2))(x)
    
    x = Conv2D(8, (5, 5), kernel_initializer='he_uniform', padding='same')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = UpSampling2D(size=(2,2))(x)
    
    x = Conv2D(8, (7, 7), kernel_initializer='he_uniform', padding='same')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    
    x = Conv2D(4, (5, 5), kernel_initializer='glorot_normal', padding='same')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    
    x = Conv2D(1, (2, 2), kernel_initializer='glorot_normal', padding='same')(x)
    x = BatchNormalization()(x)
...
def discriminator():
    ...
    image = Input(shape=(20,20,1))
    
    x = Conv2D(16, (8, 8), padding='same')(image)
    x = BatchNormalization()(x)
    x = LeakyReLU(0.1)(x)
    x = Dropout(0.2)(x)
    
    x = Conv2D(8, (6, 6), kernel_initializer='glorot_normal', padding='same', strides=(2,2))(x)
    x = BatchNormalization()(x)
    x = LeakyReLU(0.2)(x)
    x = Dropout(0.2)(x)
    
    x = Conv2D(4, (4, 4), kernel_initializer='glorot_normal', padding='same', strides=(2,2))(x)
    x = BatchNormalization()(x)
    x = LeakyReLU(0.2)(x)
    x = Flatten()(x)
...
def generate_samples(n=0, save=True):
    ...
    for c in range(3):
        rr.append(
            np.concatenate(generated_images[c * 3:(1 + c) * 3]).reshape(60, 20))
    img = np.hstack(rr)
